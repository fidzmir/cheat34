...
let REFRESH_TOKEN = "7t3zgivo5f7v"; // ✅ Token baru lu aman di sini
...
// 🎯 FIX UTAMA: Mencari ID Active XI langsung dari tabel lineup eksternal server Supabase
async function dapatkanIdSkuadUtama() {
    const idStarter = new Set();
    try {
        // Kita coba scan tabel database umum game ini: 'lc_lineup' atau 'lc_squad'
        let res = await fetch(`${SUPA_URL}/rest/v1/lc_lineup?select=*`, { headers: HEADERS_SILUMAN });
        if (!res.ok) {
            res = await fetch(`${SUPA_URL}/rest/v1/lc_squad?select=*`, { headers: HEADERS_SILUMAN });
        }
        
        if (res.ok) {
            const data = await res.json();
            data.forEach(p => {
                if (p.player_id) idStarter.add(Number(p.player_id));
            });
        }
    } catch (e) {
        console.log(`⚠️ [WARNING] Gagal memetakan skuad utama lewat API DB.`);
    }
    return idStarter;
}
...
async function jalankanLoopMatchGame() {
    console.log(`\n\x1b[35m⚽ [ENGINE] PC LOKAL MODE V29 RUNNING...\x1b[0m\n`);
    while (true) {
        console.log(`--------------------------------------------------`);
        ...
        try {
            ...
            // Ambil data skuad utama terpisah dari server, lalu amankan dari SBC
            const skuadUtama = await dapatkanIdSkuadUtama();
